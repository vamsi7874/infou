...
exports.getWeatherData = async (req: any, res: any) => {
  ...
  const daily =
    "temperature_2m_max,temperature_2m_min,rain_sum,wind_speed_10m_max,wind_direction_10m_dominant";
  const hourly =
    "rain,showers,snowfall,cloud_cover,cloud_cover_low,cloud_cover_mid,cloud_cover_high,visibility";
  ...
  try {
    ...
    const weatherData = await insertOne("weather", {
      ...response?.data,
      date: moment().unix(),
    });
